...
module confreg(
    ...
    output  wire[7:0]   led,
    output  wire[31:0]  digital_num_v,
    ...
    assign digital_num_v = digital_num_v_r;
    ...
    always @ (posedge clk) begin
        if (rst == `RST_ENABLE) begin
            ...
            case(confreg_addr[15:0])
                `DIGITAL_NUM_ADDR   : conf_rdata_reg = digital_num_v_r;
    ...
    always @ (posedge clk) begin
        if(rst == `RST_ENABLE) begin
            digital_num_v_r <= 32'b0;
        end else begin
            if (write_digital_num) begin
                digital_num_v_r <= confreg_wdata;
            end
        end
    end
    ...
    always @ (posedge clk) begin
        if (rst == `RST_ENABLE) begin
            ...
            case(count[19:18])
            2'b00: begin
                scan_data1 <= digital_num_v_r[3:0];
                scan_data2 <= digital_num_v_r[19:16];
                scan_enable <= 8'b0001_0001;
            end
            2'b01: begin
                scan_data1 <= digital_num_v_r[7:4];
                scan_data2 <= digital_num_v_r[23:20];
                scan_enable <= 8'b0010_0010;
            end
            2'b10: begin
                scan_data1 <= digital_num_v_r[11:8];
                scan_data2 <= digital_num_v_r[27:24];
                scan_enable <= 8'b0100_0100;
            end
            2'b11: begin
                scan_data1 <= digital_num_v_r[15:12];
                scan_data2 <= digital_num_v_r[31:28];
